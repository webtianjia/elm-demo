<style type="text/less" scoped lang="less">
    ...
    .bg-image(@url) {
        background-image: data-uri('@{url}@2x.png');
        @media (-webkit-min-device-pixel-ratio: 3) , (min-device-pixel-ratio: 3) {
            background-image: data-uri('@{url}@3x.png');
        }
    }
    ...
    export default {
        ...
        methods: {
            ...
            _initScroll(){ /*初始化滚动，绑定DOM*/
                ...
                this.foodsScroll.on('scroll', (pos) => { /*绑定事件监听*/
                    if (pos.y <= 0) {
                        this.scrollY = Math.abs(Math.round(pos.y))
                    }
                })
            },
            _calculateHeight(){/*计算所有item的高度*/
                let foodList = this.$refs.foodList;
                let height = 0;
